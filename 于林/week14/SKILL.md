...
**作用**：获取股票的日波动与周波动数据，将两种波动绘制在同一张图表中进行对比； 根据波动大小、趋势强度，自动给出最佳买入、卖出时间建议，辅助短线/波段交易决策。

- 根据股票代码获取日K线与周K线数据 

- 计算每日涨跌幅（日波动）与每周涨跌幅（周波动）

- 将日波动、周波动绘制在同一张图中展示趋势对比 

- 根据波动大小判断：  

  - 日波动大、周趋势向上 → 日线回调为最佳买入点 

  - 日波动大、周趋势向下 → 日线反弹为最佳卖出点 
  - 日波动小、周波动小 → 观望 
  - 日波动与周波动同步放大 → 趋势明确，顺势交易 
  - 止损：最近一根标志性阳线最低点

- 输出最终图表 + 买卖建议

> 示例: 已为你生成股票 600000 的日波动与周波动对比图。 当前趋势：上升 日波动幅度：1.82% 周波动幅度：3.15% 最佳买入时间：日线回调至低位时 最佳卖出时间：日线连续大涨冲高回落时 支撑位：7.82，压力位：9.50推荐输出结果
