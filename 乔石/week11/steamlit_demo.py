...
def mcp_my_tool_callable_filter(context, tool):
    return tool.name == "get_multiply" or tool.name == "get_text_length" or tool.name == "get_current_time"
...
async def get_model_response3(prompt, model_name, use_tool):
    ...
    mcp_server3 = MCPServerSse(
        ...
        params={"url": "http://localhost:8900/sse"},
        cache_tools_list=False,
        tool_filter=mcp_my_tool_callable_filter,
        client_session_timeout_seconds=20,
    )
    ...
    async with mcp_server2, mcp_server3:
        ...
        if use_tool:
            ...
            my_tool_agent = Agent(
                name="Tool Assistant",
                instructions="Solve task, like 计算两个数字的乘积 统计文本的总字符数 获取当前日期时间",
            ...
            agent = Agent(
                name="triage_agent",
                instructions="Handoff to the appropriate agent based on the language of the request. 如果是计算数字相乘、获取当前时间、获取文本长度任务，转交给 my_tool_agent",
                handoffs=[news_agent, my_tool_agent],
                model=OpenAIChatCompletionsModel(
                    model=model_name,
                    openai_client=external_client,
                ),
