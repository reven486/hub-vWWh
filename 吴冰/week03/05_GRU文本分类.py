...
# texts = ['还有双鸭山到淮阴的汽车票吗13号的', '从这里怎么回家']
string_labels = dataset[1].tolist()
#string_labels = 'Travel-Query', 'Travel-Query', 'Music-Play']
...
class GRUClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # 若 batch size=2，seq length=3，则形状为 (2, 3)，可能对应两个句子，每个句子3个单词的ID;
        # embedding_dim=128表示每个单词被表示为一个128维的实数向量。
        embedded = self.embedding(x)

        # batch size * seq length * embedding_dim -》 batch size * seq length * hidden_dim
        # gru_out: 每个时间步的隐藏状态 [batch_size, seq_len, hidden_size]
        # final_hidden_state: 最后一个时间步的隐藏状态 [num_layers * num_directions, batch_size, hidden_size]
        gru_out, final_hidden_state = self.gru(embedded)
