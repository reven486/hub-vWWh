---
name: 股票信息和股价查询
description: 通过调用第三方接口 https://api.autostock.cn 获取股票、指数、板块、K线等数据，并支持基础的周波动、日波动可视化和买卖建议。
---
...
## 三、波动可视化与简单择时
**作用**：把股票的周波动、日波动放在一张图中观察，并用一个最基础的规则给出买卖时间建议。

### 波动定义
- **日波动**：`(high - low) / close * 100`
- **周波动**：`(high - low) / close * 100`
...
### 最基础建议规则
- **买入建议**：最新日波动 < 最近20日日波动均值，且最新周波动 < 最近8周周波动均值  
  说明短期和中期波动都偏低，可视为相对平稳，适合观察低吸或分批买入
- **卖出建议**：最新日波动 > 最近20日日波动均值 * 1.5，且最新周波动 > 最近8周周波动均值 * 1.2  
  说明短期和中期波动都明显放大，可视为风险上升，适合考虑卖出或减仓
- **观望建议**：其他情况  
  说明信号不够明确，先观望
...
def build_volatility_chart(day_data, week_data, code="000001"):
    day_df = pd.DataFrame(day_data).copy()
    week_df = pd.DataFrame(week_data).copy()
    ...
    day_df["volatility"] = (day_df["high"] - day_df["low"]) / day_df["close"] * 100
    week_df["volatility"] = (week_df["high"] - week_df["low"]) / week_df["close"] * 100

    latest_day = float(day_df["volatility"].iloc[-1])
    latest_week = float(week_df["volatility"].iloc[-1])
    avg_day = float(day_df["volatility"].tail(20).mean())
    avg_week = float(week_df["volatility"].tail(8).mean())

    if latest_day < avg_day and latest_week < avg_week:
        advice = "买入"
        reason = "近期日波动和周波动都低于各自均值，适合基础低吸。"
    elif latest_day > avg_day * 1.5 and latest_week > avg_week * 1.2:
        advice = "卖出"
        reason = "近期日波动和周波动同时放大，适合基础减仓或卖出。"
    else:
        advice = "观望"
        reason = "当前波动处于中间区域，暂无明确买卖信号。"

    plt.figure(figsize=(12, 6))
    plt.plot(day_df["date"], day_df["volatility"], label="日波动", color="tab:blue")
    plt.plot(week_df["date"], week_df["volatility"], label="周波动", color="tab:red")
    plt.title(f"{code} 周波动与日波动对比图")
    plt.xlabel("日期")
    plt.ylabel("波动(%)")
    plt.legend()
    ...
    return {
        "code": code,
        "latest_day_volatility": round(latest_day, 2),
        "avg_20d_volatility": round(avg_day, 2),
        "latest_week_volatility": round(latest_week, 2),
        "avg_8w_volatility": round(avg_week, 2),
        "advice": advice,
        "reason": reason,
    }
```
